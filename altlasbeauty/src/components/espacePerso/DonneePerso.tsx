...
export default function DonneePerso() {
  ...
  if (!patientData) return <div className="p-4">Aucune donnée patient disponible</div>
    ...
    <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <InfoItem 
            label="Âge" 
            value={patientData.annee_naissance 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <div>
          ...
          <InfoItem label="Taille" value={patientData.taille ? `${patientData.taille} cm` : undefined} />
          <InfoItem label="Poids" value={patientData.poids ? `${patientData.poids} kg` : undefined} />
        ...
        {/* Demande de devis */}
        {patientData.demandeDevis && (
          <div>
            <SectionTitle>Demande de devis</SectionTitle>
            <InfoItem label="Statut" value={patientData.demandeDevis.status} />
            <InfoItem label="Intervention principale" value={patientData.demandeDevis.intervention_1} />
            <InfoItem label="Intervention secondaire" value={patientData.demandeDevis.intervention_2} />
            {patientData.demandeDevis.date_souhaite && (
              <InfoItem 
                label="Date souhaitée" 
                value={new Date(patientData.demandeDevis.date_souhaite).toLocaleDateString('fr-FR')} 
              />
            )}
          </div>
        )}
      ...
      {patientData.devis && (
        <div className="mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <InfoItem label="Signé" value={patientData.devis.is_signed} isBoolean />
            {patientData.devis.date_operation && (
              <InfoItem 
                label="Date d'opération" 
                value={new Date(patientData.devis.date_operation).toLocaleDateString('fr-FR')} 
              />
            )}
            {patientData.devis.date_sejour && (
              <InfoItem 
                label="Date de séjour" 
                value={new Date(patientData.devis.date_sejour).toLocaleDateString('fr-FR')} 
              />
            )}
          </div>
        </div>
...
function InfoItem({ 
  ...
  const formatValue = () => {
    if (value === undefined || value === null) return <span className="text-gray-400">Non renseigné</span>
